...
9. **Instant Search Indexing Pipeline Invariant (Google Indexing API & IndexNow):**
   - **Bảo mật Chìa khóa Service Account:** File khóa `google-indexing-key.json` BẮT BUỘC nằm trong `.gitignore`, tuyệt đối không commit lên Git hay để lộ credentials. Tài khoản dịch vụ phải được cấp quyền **Chủ sở hữu (Owner)** trong Google Search Console.
   - **Tệp Khóa IndexNow:** Tệp xác thực `website/src/a0e5b1274f8c49d89326d18a39b4f7e2.txt` BẮT BUỘC duy trì tại root và được `website/build.ps1` sao chép sang `website/dist/`.
   - **Tự Động Kích Hoạt Sau Deploy:** Sau khi hoàn tất triển khai Cloudflare Pages (`wrangler pages deploy`), Agent BẮT BUỘC kích hoạt gửi yêu cầu lập chỉ mục qua `scripts/google-index.mjs` và `scripts/indexnow.mjs` (hoặc `npm run index:all`), bảo đảm bài viết mới/cập nhật được Googlebot và ChatGPT Search nhận diện trong vòng vài giây.
10. **AI Copilot Drawer & WebChat Widget Integration Protocol (Launcher Separation & Event Delegation Invariant):**
   - **Zero Floating Clutter & Launcher Separation:** Khi tích hợp widget chat hoặc bảng hỏi AI vào các trang đã có sẵn nút Floating Action Badge ("🤖 Trải Nghiệm Thử Bot Zalo AI"), BẮT BUỘC cấu hình thuộc tính `data-hide-launcher="true"` trong thẻ `<script>` nhúng để ẩn hoàn toàn nút tròn nổi ở góc màn hình, tránh tình trạng đè nút gây ức chế thị giác.
   - **Event Delegation Semantic Triggers (`[data-zf-open]`):** Bảng hỏi AI chỉ được kích hoạt mở thông qua các phần tử điều hướng ngữ nghĩa có gắn cờ `data-zf-open`:
     - Desktop Navbar: Thẻ `<button class="btn btn-ask-ai" data-zf-open title="...">✨ Hỏi AI</button>` (trang tiếng Anh dùng `✨ Ask AI`).
